...
if ($confirm_sesskey = confirm_sesskey()) {
    ...
    if ($commissionid && $action) {
        ...
        redirect(new moodle_url('/local/referral/dashboard.php'));
    }
...
foreach ($withdrawals as $wr) {
    ...
    if ($wr->status == 0) {
        $wractions .= html_writer::link(
            new moodle_url('/local/referral/dashboard.php', [
                'withdrawalid' => $wr->id, 'action' => 'wdone', 'sesskey' => sesskey()
            ]),
            'تم الدفع ✅',
        ...
        $wractions .= html_writer::link(
            new moodle_url('/local/referral/dashboard.php', [
                'withdrawalid' => $wr->id, 'action' => 'wreject', 'sesskey' => sesskey()
            ]),
            'رفض ❌',
